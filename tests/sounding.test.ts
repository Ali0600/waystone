...
describe('nearestBuried', () => {
  ...
  it('answers with the nearest unfound cache in range only', () => {
    ...
    const sys = new DiscoverySystem(
      ...
      state,
      new EventBus(),
      { lantern: true, grapple: false, sounding: true, chime: false, mistwalker: false, ferry: false },
      () => 0,
    )
  ...
  it('digging a buried cache is a normal interact once the rod is carried', () => {
    const state = createInitialState()
    const caps = { lantern: true, grapple: false, sounding: false, chime: false, mistwalker: false, ferry: false }
