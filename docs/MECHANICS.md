...
- [x] **GLB hero — the Surveyor (M43)** — a `?char=glb` / Attunement toggle swaps the procedural hero for **Waystone's own Blender-authored hooded Surveyor**, driven over an `AnimationMixer` via the same `IHeroCharacter` seam. The mesh is original; its **skeleton and 13 animation clips are inherited from the CC0 KayKit "Adventurers" rig**, so no animation had to be authored — the whole contract is keeping the clip names (`Idle`/`Running_A`/`Jump_Idle`, the 1H melee swings) and the `handslot.l`/`handslot.r` bones. Lantern rides the left hand; **combat follows the same toggle (M41)** with a CC0 KayKit sword on the right, renormalized to a sane blade length. Rebuild the asset with `tools/blender/build_waystone_hero.py` · `tests/glbanim.test.ts` · `browser-QA`
...
- [x] **Maps** (`M` isle · `N` world) — the local isle view (titled by the isle, zoomed in) and the World Map (all islands + latent ghosts); M/N switch while open, same key closes; framing math is pure. Markers: **?** to reach, **○** hollow ring = collected, **☺** resident drawn at their hub home (a recruited person is dropped from the meeting spot), ⚓/◎/≋; a legend under the canvas · `tests/map.test.ts` · `browser-QA`
- [x] **Minimap** — an always-on circular top-left compass of the current isle (north-up, same ?/○/☺/⚓ semantics as the maps via the shared pure helpers); hidden during a duel with the rest of the world HUD · `tests/map.test.ts` (shared framing/markers) · `browser-QA`
...
- [x] **Command menu** — classic JRPG box (Attack / Glyphs / Defend / Item): arrows **or WASD** navigate, **Enter or Space** confirms/descends, **Esc** backs out, with cursor memory · `tests/menu.test.ts`
